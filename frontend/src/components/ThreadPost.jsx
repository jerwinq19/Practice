...
const ThreadPost = ({ thread }) => {
  ...
  return (
    <div
      key={thread.id}
      className=" bg-white rounded-xl w-full lg:w-2/5 p-5 border-2 border-gray-300 shadow-xl flex flex-col gap-5"
    >
      <div className="flex flex-row gap-3 justify-between items-center">
        <div className="flex flex-row gap-3 items-center">
          ...
          <h1 className="text-1xl font-bold text-gray-800">
            {thread.author_name}{" "}
            {thread.is_annony && user.pk === thread.author ? "(You)" : ""}
          </h1>
        </div>

        <button
          className={`group relative rounded-full p-1 border border-gray-300 pointer-cursor  ${
            user.pk === thread.author ? "" : "hidden"
          }`}
        >
          <img
          ...
          <div className="absolute top-full right-0 rounded-lg p-3 mt-1 shadow-md scale-y-0 group-focus:scale-y-100 origin-top bg-white border border-gray-300 flex flex-col gap-2">
            <div
              className="hover:bg-gray-300 rounded px-4"
      ...
      <div className="py-5 flex flex-col gap-5">
        <h1 className="text-xl">
          CATEGORY: <span className="font-bold">{thread.category}</span>
        </h1>
        <div>
          <h1 className="font-bold text-xl">{thread.title}</h1>
          <h1 className="line-clamp-5 mt-2">{thread.content}</h1>
        </div>
      </div>
      ...
      {comment && (
        <CommentInput authorID={user.pk} threadID={thread.id} toast={toast} />
      )}
    </div>
  );
};
